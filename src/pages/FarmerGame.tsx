...
export function FarmerGame() {
  ...
  const applyLocalMove = async (
    ...
    if (res.result.promoted && animationsEnabled) {
  ...
  const checkBoardEnd = (result: MoveResult) => {
    if (result.winner) {
      const outcome: GameOutcome = result.winner === 'w' ? 'white' : 'black';
      const reason: GameEndReason =
        result.winReason === 'promotion' ? 'promotion' :
        result.winReason === 'pawns-cleared' ? 'pawns-cleared' :
        'queen-captured';
      finalize({ outcome, reason });
      return;
    }
    if (result.stalemate) finalize({ outcome: 'draw', reason: 'stalemate' });
  };
  ...
  const attemptMove = (from: Square, to: Square, viaClick = false): boolean => {
    void applyLocalMove(from, to, viaClick);
    return true;
  };
