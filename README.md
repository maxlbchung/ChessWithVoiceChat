...
| **Secret Queen** | Before move one, each side secretly crowns one of its 8 pawns. The fake moves like a queen from ply 1 but renders as an ordinary pawn to the opponent for as long as it also moves like one — single steps, the home-rank double-step, diagonal captures, even en passant (in both directions: it can capture ep and be ep-captured). The moment it makes a move a pawn couldn't (or gives a non-pawn check, or reaches the last rank, or is captured) the disguise drops for good. In free play / sandbox you pick both fakes and see them both, marked with the owner-shadow pawn. |
...
src/
  lib/
    ...
    sweeperChess.ts      # Chesssweeper engine — minefield + reveal on top of chess.js
  components/
  ...
  pages/
    ...
    Game.tsx, MergeGame.tsx, TwoGame.tsx, CashGame.tsx, HeroGame.tsx, SweeperGame.tsx
...
TBD — see the discussion in [LICENSE](LICENSE) once it's added. The intent is **non-commercial**: source-available so anyone can fork, learn, and self-host, but the custom modes (Merge, Two, Cash Money, Hero, Chesssweeper) shouldn't be lifted and monetized. Game *rules* aren't copyrightable, but the implementation and assets are.
